...
function fetchProgress() {
    fetch('server_controller/get_course_update.php?user_id=<?php echo $_SESSION["user_id"]; ?>&course_id=<?php echo $course_id; ?>')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                updateProgressUI(data);
            } else {
                console.error('Failed to fetch updated progress: ' + data.message);
                showToast(data.message || 'Failed to fetch progress', 'error');
            }
        })
        .catch(err => {
            console.error('Error fetching progress:', err);
            showToast('Error fetching progress', 'error');
        });
}
...
function updateProgressUI(data) {
    ...
    // Update check icons inside buttons
    ...
    // Update the completion status form or message
    ...
    completionContainers.forEach(container => {
        ...
        if (isCompleted) {
            ...
            `;
            // Re-bind the event listener to new form
            container.querySelector('form.mark-complete-form').addEventListener('submit', function(e) {
                e.preventDefault();
                markModuleAsWatched(videoId, data.course_id);
            });
        }
    });

    // Optionally enable quiz button if all modules are watched
// Update quiz button dynamically
